# OneC Backup Bot

Telegram-бот для **ручного** резервного копирования файловой базы 1С через команды в Telegram.

## Возможности
- ✅ Выгрузка базы 1С в `.dt` через `DESIGNER /F ... /DumpIB` (поддержка авторизации `/N` и `/P`)
- ✅ Дополнительная упаковка в ZIP (настраиваемая степень сжатия)
- ✅ Организация копий по датам (папка для каждого дня: `YYYY-MM-DD`)
- ✅ Telegram-бот: `/backup`, `/status`, `/health`, `/lastlog`
- ✅ **Расширенный мониторинг:** CPU, RAM, Disk, Network, RDP сессии, процессы
- ✅ **Интеграция с Grafana:** Prometheus, InfluxDB, Loki
- ✅ Защита от параллельных бэкапов (глобальный lock) и таймаут дампа
- ✅ Определение изменений в базе (fingerprint) — пропуск бэкапа, если нет изменений

## Режим работы

**Только ручные резервные копии** — автоматическое расписание удалено. Копии создаются исключительно по команде `/backup` в Telegram.

## Быстрый старт

### 1. Требования
- Windows Server 2019/2022, Python 3.10+
- 1С 8.3 (тонкий клиент) — путь к `1cv8.exe`
- Telegram бот (токен от @BotFather)

### 2. Установка
```powershell
python -m venv .venv
.\.venv\Scripts\Activate.ps1
pip install -r requirements.txt
```

### 3. Настройка

Скопируйте `.env.example` в `.env` и заполните:
```env
BOT_TOKEN=your_bot_token_from_botfather
ALLOWED_USER_IDS=123456789,987654321

# Пути (можно указать в .env или config.yaml)
ONEC_EXE=C:\Program Files (x86)\1cv8\8.3.24.1368\bin\1cv8.exe
ONEC_BASE_PATH=D:\1C_Bases\MyDatabase
BACKUP_DIR=D:\1C_Backups

# Опционально: учетные данные 1С
ONEC_UC=Администратор
ONEC_UP=пароль

# Опционально: интервал метрик (используется внутренним сборщиком)
METRICS_INTERVAL=60
```

Отредактируйте `config.yaml`:
- `onec.exe` — путь к `1cv8.exe`
- `onec.base_path` — папка с базой 1С (где `.1CD`)
- `backup.backup_dir` — каталог для бэкапов
- `backup.compress` — `zip` или `none`

### 4. Запуск
```powershell
python main.py
```

В Telegram отправьте боту:
- `/help` — список команд
- `/backup` — создать резервную копию
- `/status` — последние результаты
- `/health` — состояние системы

## Структура папок

Резервные копии сохраняются по датам:
```
backup_dir/
├── 2025-01-15/
│   ├── Zernosbyt_2025-01-15_10-30-45.zip
│   └── Zernosbyt_2025-01-15_14-15-30.zip
├── 2025-01-16/
│   └── Zernosbyt_2025-01-16_09-00-12.zip
├── backup.log
└── app.sqlite3
```

## Команды бота

| Команда | Описание |
|---------|----------|
| `/backup` | Создать резервную копию |
| `/status` | Показать последние 20 бэкапов |
| `/health` | CPU, RAM, Disk, последний успешный бэкап |
| `/lastlog` | Получить файл с последними 100 строками лога |

## Запуск как сервис

Подробнее см. `OPERATIONS.md`. Краткий вариант:
```powershell
nssm install OneCBackupBot "C:\path\.venv\Scripts\python.exe" "C:\path\main.py"
nssm set OneCBackupBot AppDirectory "C:\path\to\project"
nssm start OneCBackupBot
```

## Структура проекта

- `main.py` — точка входа
- `onec_backup_bot/backup.py` — логика бэкапа
- `onec_backup_bot/bot.py` — telegram-команды
- `onec_backup_bot/config.py` — конфигурация
- `onec_backup_bot/db.py` — SQLite история
- `onec_backup_bot/logger.py` — логирование
- `config.yaml` — настройки

## Безопасность

- Пароли 1С в логах маскируются (`***`)
- `.env` не коммитится (см. `.gitignore`)
- Доступ к боту только для указанных Telegram ID

## Мониторинг (Grafana + Prometheus, локально)

Бот предоставляет метрики для локального **Prometheus** через эндпоинт скрейпа.

### Доступные метрики:
- **Система:** CPU, RAM, Disk, Network, Uptime
- **RDP сессии:** Количество активных пользователей (Windows)
- **Процессы:** Количество процессов, топ по CPU/RAM
- **Диск I/O:** Операции чтения/записи, throughput
- **Бэкапы:** Статус, размер, длительность

### Быстрый старт с локальным Prometheus:
1. В Prometheus добавьте конфиг скрейпа:
   ```yaml
   scrape_configs:
     - job_name: 'onec_backup_bot'
       static_configs:
         - targets: ['<SERVER_IP>:8080']
       metrics_path: /api/metrics.prom
   ```
2. В Grafana добавьте Prometheus как Data Source и строите графики/алерты.

📊 **Подробности:** см. `GRAFANA_INTEGRATION.md`

## Дополнительно

- Для автоматической отправки в S3/облако используйте внешние скрипты и Windows Task Scheduler
- Подробная документация: `OPERATIONS.md`
- Интеграция с Grafana: `GRAFANA_INTEGRATION.md`

## Лицензия

Proprietary/Internal
